...
from autoalbument.faster_autoaugment.search import FasterAutoAugmentSearcher
...
def get_prettified_cfg(cfg):
    ...
    if cfg["task"] == "classification" and "semantic_segmentation_model" in cfg:
        del cfg["semantic_segmentation_model"]
    elif cfg["task"] == "semantic_segmentation" and "classification_model" in cfg:
...
@hydra.main(config_path="../src/autoalbument/cli/conf", config_name="config", version_base="1.2")
def main(cfg):
    config_dir = get_config_dir()
    check_config_version(cfg, config_dir)
    validate_cfg(cfg)
    if config_dir is not None:
        print(f"{config_dir=} given, adding it to path")
        sys.path.append(config_dir)
    print(get_prettified_cfg(cfg))
    cwd = os.getcwd()
    print(f"Working directory: {cwd}")
    # searcher = instantiate(cfg.searcher, cfg=cfg)
    searcher = FasterAutoAugmentSearcher(cfg=cfg)
    searcher.search()
