...
def main():
    ...
    scaling = 1
    xs, ys = np.meshgrid(
        np.arange(label.shape[1]),
        np.arange(label.shape[2]),
        indexing="ij",
    )
    ...
    print(f"zs: 0: {label.shape[0] * scaling}")
    ...
    with open(label_out_path, "ab") as label_f:
        for c in tqdm(range(label.shape[0]), position=0):
            ...
            mask_channel = label_intensities > 0
            ...
            n_written_points += label_merged_array.shape[0]
    print(f"promised: {n_label_points} wrote {n_written_points} points")
