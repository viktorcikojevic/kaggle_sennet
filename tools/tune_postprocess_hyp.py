...
def objective(
    ...
    threshold: float | None = trial.suggest_float("threshold", 0.0001, 0.3, log=True)
    ...
    for folder in chunk_dirs:
        ...
        percentile_threshold = np.interp(threshold, cached_percentiles[folder.name], percentile_range)
